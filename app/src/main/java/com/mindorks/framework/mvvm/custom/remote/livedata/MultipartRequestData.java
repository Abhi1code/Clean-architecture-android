...
public class MultipartRequestData<T> extends LiveData<StateData<T>>
    ...
    public MultipartRequestData(@NonNull final Class<T> clazz,
                             @NonNull final VolleyRequestParams volleyRequestParams) {
        this.clazz = clazz;
        this.volleyRequestParams = volleyRequestParams;
        this.requestInterface = VolleyRequest.createVolleyMultipartRequest(this.volleyRequestParams,
    ...
    private class MyTaskSuccessListener implements VolleyResponse.Response<String> {
        @Override
